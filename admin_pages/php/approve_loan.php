...
if (!isset($_SESSION['UserId'])) {
    ...
    //Approve Loan
    $loanId = $_POST['loanId'];
    $query = $pdo->prepare("UPDATE Loans SET Status = 'Approved', Approved_At = Now() WHERE LoanId = ?");
    ...
    $action = "Admin User ". $_SESSION['UserId']. ' Approved Loan for Customer : '. $Loan['UserId'];
    $description = $user['FullName']. ', Banker with User ID: '. $_SESSION['UserId']. ' Approved Loan for: '. $Loan['FullName']. ' with UserID: '. $Loan['UserId']. "By doing this the banker accept that the bank branch has verified every information supplied by the customer";
