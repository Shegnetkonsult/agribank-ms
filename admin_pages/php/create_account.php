    ...
    try {
        ...
        if ($existing > 0) {
            ...
            $stmt = $pdo->prepare('INSERT INTO Users (Username, FullName, Email, PhoneNumber, NationalID, Passport, Address, Password, Role, Created_At, Status)
                                    VALUES (:username, :fullName, :email, :phoneNumber, :nationalID, :passport, :address, :password, 2, NOW(), "Active")');
            ...
            $action = 'User Created from ipAddress: '. $ipAddress;
            $description = "A new Admin user ($username: $fullName) has been created now :NOW()";
